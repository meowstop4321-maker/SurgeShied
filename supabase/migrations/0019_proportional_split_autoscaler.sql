-- Migration 0019: Proportional Split Autoscaling with Strict Invariant Verification
--
-- 1. When a lane splits, BOTH capacity and occupancy (seats_taken) are proportionally redistributed.
-- 2. Waiting queue entries on the splitting lane are also balanced across the new lane.
-- 3. Strict Safety Invariants:
--      SUM(capacity) == events.capacity
--      SUM(seats_taken) == total_registered
--      seats_taken <= capacity for every lane
-- 4. Contextual infrastructure audit logging for scaling events.

-- 1. Updated split_lane with proportional capacity & occupancy redistribution
...
-- 2. Updated merge_lanes with clean consolidation
...
-- 3. Updated rebalance_lanes with Safety Invariant Assertions
...
BEGIN
  ...
  IF v_target > v_current THEN
    WHILE v_current < v_target LOOP
      -- Select the lane with the highest capacity to split
  ...
  -- =========================================================================
  -- Safety Validation Check 1: Capacity Invariant (SUM(capacity) == events.capacity)
  -- =========================================================================
  ...
  -- =========================================================================
  -- Safety Validation Check 2: No Lane Overbooking (seats_taken <= capacity)
  -- =========================================================================
  PERFORM 1 FROM public.seat_partitions
...
-- 4. Updated simulate_surge_load: Pre-scales lanes proportionally before allocation
...
BEGIN
  ...
  -- Refresh capacity & taken after potential scale out
  SELECT coalesce(sum(capacity), 0), coalesce(sum(seats_taken), 0)
  INTO v_total_capacity, v_total_taken
  FROM public.seat_partitions
  WHERE event_id = p_event_id;
  ...
  -- 2. Balanced multi-lane allocation across all lanes with headroom
